    ...
    for i in range(num_samples):
        ...
        def lora_init_hook(m, x, y, name):
            ...
            if torch.isinf(lstsq).any() or torch.isnan(lstsq).any():
                loggging.info("Nan or inf in lstsq")
                return y
            ...
            logging.info(
                f"ori weight err: {ori_weight_err}, weight err: {weight_err}, ori act err: {ori_err}, act err: {new_err}")
            if (not torch.isnan(deq_weight).any()) and (not torch.isnan(R).any()) and (not torch.isnan(L).any()) \
                ...
                logging.info(f"A, {name}: count {lora_As[name + '.count']}")
                logging.info(f"B, {name}: count {lora_Bs[name + '.count']}")
