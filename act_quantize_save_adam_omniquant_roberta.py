...
def obtain_lora_input(lora_model, lora_model_device, input_ids, quantized_module):
    ...
    for name, m in lora_model.named_modules():
        if ".0." in name:
            print("!!!", name, quantized_module, name == quantized_module, isinstance(m, tuners.lora.Linear))
...
def main(args):
    ...
    ordered_modules = [
        "roberta.encoder.layer.0.attention.self.query", "roberta.encoder.layer.0.attention.self.key",
        "roberta.encoder.layer.0.attention.self.value", "roberta.encoder.layer.0.attention.output.dense",
        "roberta.encoder.layer.0.intermediate.dense", "roberta.encoder.layer.0.output.dense"]
